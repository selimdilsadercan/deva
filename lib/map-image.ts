...
const compressImage = (image, width = 400, quality = 50, fmt = "webp") => {
  ...
  if (image.indexOf(NOTION_HOST) === 0 && image.indexOf("amazonaws.com") > 0) {
...
const mapImgUrl = (img, block, type = "block", from) => {
  ...
  const isImgBlock = IMG_URL_TYPE === "Notion" || type !== "block";
  if (isNotionImg && isImgBlock) {
    ret = NOTION_HOST + "/image/" + encodeURIComponent(ret) + "?table=" + type + "&id=" + block.id;
  }
