...
import * as types from "@/types/notion-api";
...
export class NotionAPI {
  ...
  public async getPage(pageId: string): Promise<notion.ExtendedRecordMap> {
    ...
    while (true) {
      ...
      const newBlocks = await this.getBlocks(pendingBlockIds).then((res) => res.recordMap.block);
    ...
    // fetch signed URLs for any embedded files.
    await this.addSignedUrls({ recordMap, contentBlockIds });

    return recordMap;
  }

  public async addSignedUrls({ recordMap, contentBlockIds }: { recordMap: notion.ExtendedRecordMap; contentBlockIds?: string[] }) {
    recordMap.signed_urls = {};
  ...
  public async getCollectionData(collectionId: string, collectionViewId: string, collectionView: any) {
    const limit = 9999;
    const searchQuery = "";
  ...
  public async fetch<T>({ endpoint, body, headers: clientHeaders }: { endpoint: string; body: object; headers?: any }): Promise<T> {
    const headers: any = {
      ...clientHeaders,
